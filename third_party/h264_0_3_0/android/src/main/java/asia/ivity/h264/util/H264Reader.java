...
public class H264Reader {
  ...
  public static synchronized Bitmap decode(InputStream is, long length, int width, int height)
    ...
    int codecWidth = align16(Math.max(width, 16));
    int codecHeight = align16(Math.max(height, 16));

    Log.d(TAG, "codec resolution: " + codecWidth + " x " + codecHeight);

    OutputSurface outputSurface = new OutputSurface(codecWidth, codecHeight);

    MediaFormat format =
        MediaFormat.createVideoFormat(MediaFormat.MIMETYPE_VIDEO_AVC, codecWidth, codecHeight);
    ...
    if (bitmap != null) {
      int cropW = Math.min(width, bitmap.getWidth());
      int cropH = Math.min(height, bitmap.getHeight());
      bitmap = Bitmap.createBitmap(bitmap, 0, 0, cropW, cropH);
    }
