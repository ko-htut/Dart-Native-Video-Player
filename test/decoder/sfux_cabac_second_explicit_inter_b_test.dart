...
void main() {
  test('decodes sfux AU8 B picture and its AU7 reference continuity', () {
    // Eighth presentation frame (ninth decode-order AU), POC 14.
    ...
    // Exact VCL SHA-256:
    // 39c9d733c7e9f121b9e8a4071457ce12a5f3b7d571c0cb18c40d10c3c8a7ea3a
    final vcl = _bytes(
    ...
    expect(vcl, hasLength(353));
    expect(header.reader.bitLength, 2808);
    ...
    expect(header.picOrderCntLsb, 14);
    ...
    // AU7's MMCO 1 removes frame_num/PicNum 0, then its reference POC 12 is
    // inserted. AU8 consequently gets POC 12 at List0 index 0 and POC 16 at
    // List1 index 0. Its explicit Bi macroblock below selects both ref_idx 0.
    final afterAu7 = applyShortTermDpbMarking<int>(
      shortTermReferences: const <H264ShortTermReference<int>>[
        H264ShortTermReference(frameNum: 0, pictureOrderCount: 0, value: 0),
    ...
    expect(afterAu7.removed.single.pictureOrderCount, 0);
    expect(
      afterAu7.references.map((reference) => reference.pictureOrderCount),
      <int?>[4, 6, 10, 16, 12],
    );
    final lists = buildBReferenceLists<int>(
      shortTermReferences: afterAu7.references,
    ...
    final afterAu8 = applyShortTermDpbMarking<int>(
      shortTermReferences: afterAu7.references,
      currentPicture: const H264ShortTermReference<int>(
        frameNum: 6,
        pictureOrderCount: 14,
        value: 14,
      ),
      maxFrameNum: sps.maxFrameNum,
      maxNumRefFrames: sps.maxNumRefFrames,
    ...
    expect(afterAu8.appendedCurrentPicture, isFalse);
    expect(afterAu8.references, orderedEquals(afterAu7.references));
...
void _decodeSyntax(SliceHeader header, PpsInfo pps) {
  ...
  // FFmpeg `-debug mb_type` independently reports 3138 inferred Direct
  // (`d`), seven explicit Direct (`D`), 42 I_8x8 (`i`), 321 I_16x16
  // (`I`), one List1 (`<`), and one bi-predicted (`X`) macroblock.
  expect(eosAddress, 3509);
  expect(states, everyElement(isNotNull));
  expect(skipped, 3138);
  expect(nonSkipAddresses, hasLength(372));
  expect(inter, 2);
  expect(directMacroblocks, 7);
  expect(intra, 363);
  ...
  expect(typeAddresses[0], <int>[1425, 1426, 1502, 1503, 1504, 1580, 1582]);
  expect(typeAddresses[3], <int>[1175]);
  expect(typeAddresses[2], <int>[1659]);
  expect(typeAddresses[26], <int>[2127]);
  expect(macroblockHash.toRadixString(16), '4970df01212688ce');

  // MB1175 is B_Bi_16x16 and selects AU7 POC12 through List0 ref_idx 0;
  // MB1659 is B_L1_16x16. List1 ref_idx is inferred zero because only one
  // active reference exists, but its MVD syntax is still present.
  expect(subTypeCounts, isEmpty);
  expect(mbPartitionCount, 9);
  expect(motionPartitionCount, 3);
  expect(referenceCounts, <String, int>{'l0:0': 1, 'l1:0': 2});
  expect(mvdCounts, <String, int>{'l0:269,0': 1, 'l1:0,0': 1, 'l1:0,-396': 1});
  expect(motionRecords, <String>[
    '1175:0:0:l0:r0:269,0',
    '1175:0:0:l1:r0:0,0',
    '1659:0:0:l1:r0:0,-396',
  ]);
  expect(nonzeroMvdExamples, <String>[
    '1175:0:0:l0:r0:269,0',
    '1659:0:0:l1:r0:0,-396',
  ]);
  expect(motionHash.toRadixString(16), '1085f7f6fc286baf');

  expect(chromaModes, <int, int>{0: 361, 1: 2});
  expect(intra8Predicted, 160);
  expect(intra8Remaining, 8);
  expect(intra4Predicted, 0);
  expect(intra4Remaining, 0);
  expect(intraModeHash.toRadixString(16), '3cbc6351fa64574f');
  ...
  expect(cbpCounts, <int, int>{1: 21, 0: 342, 16: 9});
  ...
  expect(qpDeltaCounts, <int, int>{0: 372});
  expect(nonzeroQp, isEmpty);

  expect(residual.codedBlocks, <String, int>{'luma8x8': 21, 'chromaDc420': 9});
  expect(residual.coefficients, <String, int>{'luma8x8': 21, 'chromaDc420': 9});
  expect(residual.footprint, <String>[
    ...
    '1485:luma8x8:0:0:1',
    '1502:chromaDc420:1:0:-2',
    '1503:chromaDc420:1:0:1',
  ...
  expect(residual.hash.toRadixString(16), '1dcb372b5de082dc');

  expect(arithmetic.isTerminated, isTrue);
  expect(arithmetic.bitPosition, 2806);
  expect(arithmetic.range, 460);
  expect(arithmetic.offset, 461);
