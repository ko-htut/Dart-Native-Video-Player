...
void main() {
  test('decodes exact sfux first L0/L1 8x16 reference B AU91', () {
    // Ninety-second decode-order picture, presentation frame n=90 / POC 180.
    ...
    // video PTS 402276 at 90 kHz. Authoritative FFmpeg `-f framehash
    // -hash sha256` presentation-frame n=90 SHA-256:
    ...
    final vcl = _bytes(
      '',
    );
    expect(vcl, hasLength(133));
    expect(
      sha256.convert(vcl).toString(),
      '60dfb90c08b657c2944716b8a7777dbd33e1cfffe51d73ba31c92d1df8c067d8',
    );
    ...
    expect(header.reader.bitLength, 1024);
    ...
    expect(header.picOrderCntLsb, 52);
    ...
    // from AU0 through AU90, including all reference B pictures and wraps.
    ...
    expect(poc.picOrderCntMsb, 128);
    expect(poc.picOrderCntLsb, 52);
    expect(poc.pictureOrderCount, 180);

    final reference162 = H264ShortTermReference<String>(
      frameNum: 15,
      pictureOrderCount: 162,
      value: 'AU78/POC162',
    );
    final reference170 = H264ShortTermReference<String>(
      frameNum: 1,
      pictureOrderCount: 170,
      value: 'AU82/POC170',
    );
    final reference166 = H264ShortTermReference<String>(
      frameNum: 2,
      pictureOrderCount: 166,
      value: 'AU83/POC166',
    );
    final reference178 = H264ShortTermReference<String>(
      frameNum: 3,
      pictureOrderCount: 178,
      value: 'AU86/POC178',
    );
    final reference174 = H264ShortTermReference<String>(
      frameNum: 4,
      pictureOrderCount: 174,
      value: 'AU87/POC174',
    );
    final reference184 = H264ShortTermReference<String>(
      frameNum: 5,
      pictureOrderCount: 184,
      value: 'AU90/POC184',
    );
    final beforeAu91 = <H264ShortTermReference<String>>[
      reference162,
      reference170,
      reference166,
      reference178,
    ...
    final lists = buildBReferenceLists<String>(
      shortTermReferences: beforeAu91,
    ...
    expect(identical(lists.list0[0], reference178), isTrue);
    expect(identical(lists.list0[1], reference174), isTrue);
    expect(identical(lists.list0[2], reference170), isTrue);
    expect(identical(lists.list0[3], reference166), isTrue);
    expect(identical(lists.list0[4], reference162), isTrue);
    expect(identical(lists.list1.single, reference184), isTrue);
    ...
    expect(identical(firstMmco.removed, reference162), isTrue);
    expect(
      firstMmco.remaining.map((reference) => reference.pictureOrderCount),
      <int?>[170, 166, 178, 174, 184],
    );
    ...
    expect(secondMmco.picNumX, 2);
    expect(identical(secondMmco.removed, reference166), isTrue);
    expect(
      secondMmco.remaining.map((reference) => reference.pictureOrderCount),
      <int?>[170, 178, 174, 184],
    );
    final afterAu91 = applyShortTermDpbMarking<String>(
      shortTermReferences: beforeAu91,
    ...
    expect(afterAu91.appendedCurrentPicture, isTrue);
    expect(afterAu91.removed, <H264ShortTermReference<String>>[
      reference162,
      reference166,
    ]);
    expect(
      afterAu91.references.map((reference) => reference.pictureOrderCount),
      <int?>[170, 178, 174, 184, 180],
    );
    expect(identical(afterAu91.references[0], reference170), isTrue);
    expect(identical(afterAu91.references[1], reference178), isTrue);
    expect(identical(afterAu91.references[2], reference174), isTrue);
    expect(identical(afterAu91.references[3], reference184), isTrue);
    expect(identical(afterAu91.references[4], current), isTrue);
...
void _decodeSyntax(SliceHeader header, PpsInfo pps) {
  ...
  expect(intra, 8);
  expect(typeCounts, <int, int>{2: 3, 1: 33, 23: 8, 9: 1, 0: 1});
  expect(typeAddresses, <int, List<int>>{
    2: <int>[1188, 1835, 1890],
    1: <int>[
      1282,
      1344,
      1350,
    ...
    23: <int>[1345, 1444, 1670, 1739, 1813, 1817, 1892, 1981],
    9: <int>[1656],
    0: <int>[1661],
  });
  expect(macroblockHash.toRadixString(16), '27caa06098f7c54a');

  // Normalized from FFmpeg's independent -debug mb_type map for
  // presentation frame n=90. Every address and category matches this syntax
  // walk; the full-map FNV-1a hash is 2f282f7b26d296aa.
  ...
  expect(ffmpegMacroblockMapHash.toRadixString(16), '2f282f7b26d296aa');

  expect(subTypeCounts, isEmpty);
  expect(subTypeRecords, isEmpty);
  expect(mbPartitionCount, 39);
  ...
  expect(motionHash.toRadixString(16), '-25264749139ed923');
  expect(targetCheckpoints, <String>[
    '1656:start=484/443/442:type9=498/480/78',
    '1656:prediction=500/402/315',
    '1656:cbp16=503/452/309',
    '1656:transform0=503/452/309',
    '1656:qp-13/qpy17=513/368/320',
    '1656:residual=529/257/7',
    '1656:eos0=530/510/15',
  ]);

  expect(chromaModes, <int, int>{1: 2, 0: 4, 2: 2});
  expect(intra8Predicted, 27);
  expect(intra8Remaining, 5);
  expect(intra4Predicted, 0);
  expect(intra4Remaining, 0);
  expect(intraModeHash.toRadixString(16), '7088bdbec08fcf5a');
  ...
  expect(cbpCounts, <int, int>{0: 31, 16: 13, 1: 1, 17: 1});
  ...
  expect(residual.codedBlocks, <String, int>{'chromaDc420': 16, 'luma8x8': 2});
  expect(residual.coefficients, <String, int>{'chromaDc420': 17, 'luma8x8': 3});
  expect(residual.macroblockAddresses, <int>{
    1344,
    1345,
    1350,
    1422,
  ...
  expect(residual.footprint, <String>[
    '1344:chromaDc420:0:0:2',
    '1345:luma8x8:0:0:2',
    '1350:chromaDc420:0:0:2',
    '1422:chromaDc420:0:0:1',
    '1428:chromaDc420:0:0:2',
    '1500:chromaDc420:0:0:1',
    '1500:chromaDc420:1:0:-1',
    '1583:chromaDc420:0:0:1',
    '1584:chromaDc420:0:0:1',
    '1656:chromaDc420:0:0:3,1:3',
    '1657:chromaDc420:0:0:6',
    '1661:chromaDc420:0:0:6',
    '1661:chromaDc420:1:0:-2',
    '1662:chromaDc420:0:0:1',
    '1813:chromaDc420:0:0:2',
    '1816:luma8x8:0:0:1,2:1',
    '1816:chromaDc420:0:0:2',
    '1891:chromaDc420:0:0:2',
  ]);
  expect(residual.hash.toRadixString(16), '46b8e85f3c0d6cba');

  expect(arithmetic.isTerminated, isTrue);
  expect(arithmetic.bitPosition, 1019);
  expect(arithmetic.range, 471);
  expect(arithmetic.offset, 471);
